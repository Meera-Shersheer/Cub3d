...
/*   Updated: 2025/11/06 18:26:09 by mshershe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_additional_keys(mlx_key_data_t keydata, void *param)
{
	...
	handle_additional_keys2(keydata, g);
}
...
static void	handle_movement_keys(t_game *g)
{
	...
	if (mlx_is_key_down(g->mlx, MLX_KEY_RIGHT))
		g->player->angle += g->player->rot_speed;
...
static void	update_game_state(t_game *g)
{
	if (g->won == 1)
	{
		...
		print_win_header(g->game_time_end - g->game_time_start);
		mlx_close_window(g->mlx);
		return ;
	}
